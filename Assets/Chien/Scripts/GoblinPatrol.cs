...
public class GoblinPatrol : MonoBehaviour
{
    ...
    [Range(0f, 1f)]
    public float walkVolumeMultiplier = 0.3f;   // 🟢 âm lượng bước chân nhỏ hơn
    ...
    // 🟢 Chỉ phát tiếng đi bộ khi goblin ĐANG DI CHUYỂN + player ở gần
    void PlayWalkSound()
    {
        ...
        // 🟢 Nếu đứng yên HOẶC đang Attack → tắt sạch tiếng luôn
        if (Mathf.Abs(rb.velocity.x) < 0.1f || currentState == FrogState.Attack)
        {
        ...
        // 🟡 Nếu player ngoài soundRange → giảm dần âm lượng rồi tắt
        ...
        // 🟢 Nếu đang phát → cập nhật âm lượng theo khoảng cách và walkVolumeMultiplier
        if (audioSource.isPlaying)
        {
            float volume = Mathf.Clamp01(1 - (distance / soundRange)) * walkVolumeMultiplier;
            audioSource.volume = volume;
            return;
        }

        // 🟢 Cooldown giữa mỗi bước chân
        if (Time.time - lastWalkSoundTime < walkSoundInterval)
            return;

        // 🟢 Phát bước chân mới
        float startVolume = Mathf.Clamp01(1 - (distance / soundRange)) * walkVolumeMultiplier;
